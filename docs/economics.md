...
本文档面向使用 V3_EMA 进行经济分析的研究者，简述 Victoria 3 的经济运行机制，并明确 V3_EMA 在工具计算中所采用的简化假设。所有指标定义以本文档为准；具体实现见 `v3_ema/analysis/`。
...
V3_EMA 显式枚举此空间（去除「所有权」类 PMG 后约 1568 个组合行），用以回答：「这个建筑在某个具体配置下经济产出如何？」
...
V3_EMA `diff` 命令在保留 A1–A4 假设下做严格的**结构差分**：识别新增/移除的 (建筑 × 组合) 单元，以及在双方都存在的单元中数值变化超过 ε 的字段。这不是经济仿真意义上的「价格变动」，而是**模组/补丁层面的脚本变更检测**，适合：
